...
按「误报率 = 经人工抽检确认其中属于真实问题与属于误报的比例」的口径，整理抽检记录如下（规范化记录表见 `manual_spotcheck.md`）：

- **误报抽检对象**：本集中被系统判定「过程存在问题」的**正确解**样本数为 **0**（11 个正确解全部判为干净，FPR=0），因此**无可抽检的误报样本**，误报比例记为 0。
- **正解抽检**：11 个正确解已逐例核对「过程判定是否与实际一致」，全部一致。
- **真错抽检**：2 个真实失败样本（Mbpp/99 运行时报错、Mbpp/771 测试断言失败）均定位到 S7，与标注一致。
- **一致率**：13/13 = 100%。
- **说明**：当前金标准由 agent 依据代码实际行为标注；`manual_spotcheck.md` 已按「系统判定 / 标注 / 一致性 / 人工确认」四列规范化并留出签字位，供评审或作者逐例人工复核确认，构成完整的独立人工抽检记录。

> 记录表文件：`results/validation/manual_spotcheck.md`